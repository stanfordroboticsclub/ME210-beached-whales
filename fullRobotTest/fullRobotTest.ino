...
//  motor_verticalR.run();
  ...
  switch(state){
    case INIT_0:
      driveTo(VERTICAL_MOTORS, DIR_FRONT * STEPS_PER_INCH * INCHES_CORNER_TO_A);
      state = FORWARD_1;
      break;
    
    case FORWARD_1:
      if (driveDoneMoving()) {
        driveTo(HORIZONTAL_MOTORS, DIR_RIGHT * STEPS_PER_INCH * 20);
        state = RAM_1;
//        state = DROP_BALLS_2;
//        timer.reset();
      }
...
//        state = RAM_2;
    ...
    case FORWARD_5:
      if (driveDoneMoving()) {
        driveTo(HORIZONTAL_MOTORS, DIR_RIGHT * STEPS_PER_INCH * INCHES_THRU_GATE);
        state = RIGHT_6;
      }
      break;
